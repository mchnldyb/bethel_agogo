...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "addPatient_form")) {
  $insertSQL = sprintf("INSERT INTO patient (surname, firstname, middlename, dob, maritalstatus, sex, occupation, religion, postaladdress, homeaddress, email, nearestrelative, nearestrelativecontact, district, subdistrict, healthfacility, healthinsurance) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString($_POST['nearestrelative'], "text"),
                       GetSQLValueString($_POST['nearestrelativecontact'], "text"),
                       GetSQLValueString($_POST['district'], "text"),
                       GetSQLValueString($_POST['sub_district'], "text"),
                       GetSQLValueString($_POST['healthFacility'], "int"),
                       GetSQLValueString($_POST['health_insurance_number'], "text"));
	
  mysql_select_db($database_cnKloAgogo, $cnKloAgogo);
  $insert_HI=sprintf("INSERT INTO healthinsurance (insurancenumber, schemename) VALUES (%s, %s)", GetSQLValueString($_POST['health_insurance_number'], "text"), GetSQLValueString($_POST['scheme_name'], "text"));
  
  $name=$_POST['lastname']." ". $_POST['firstname'];
  $surname=$_POST['lastname'];
  //$name=GetSQLValueString($name,"text");
  
  $insert_Folder=sprintf("INSERT INTO folder(foldername) values(%s)", GetSQLValueString($name, "text"));
  

  mysql_select_db($database_cnKloAgogo, $cnKloAgogo);
  $Result1 = mysql_query($insertSQL, $cnKloAgogo) or die(mysql_error());
  
  if($_POST['radio_insurance']== "Yes"){
  $Result2 = mysql_query($insert_HI, $cnKloAgogo) or die(mysql_error());
  }
  
  $result3=mysql_query($insert_Folder, $cnKloAgogo) or die(mysql_error());
  $id=mysql_query("SELECT foldernumber from folder WHERE foldername LIKE '%$name%'", $cnKloAgogo) or die(mysql_error());
  $data=mysql_fetch_assoc($id);
  $foldernum=$data['foldernumber'];
  $query=mysql_query("UPDATE patient SET folder='$foldernum' WHERE surname='$surname'", $cnKloAgogo) or die(mysql_error());
  
  

  $insertGoTo = "viewPatients.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_cnKloAgogo, $cnKloAgogo);
$query_rsHealthFacility = "SELECT facilityname, healthfacility.facilitynumber FROM healthfacility,patient WHERE healthfacility.facilitynumber=patient.healthfacility";
$rsHealthFacility = mysql_query($query_rsHealthFacility, $cnKloAgogo) or die(mysql_error());
$row_rsHealthFacility = mysql_fetch_assoc($rsHealthFacility);
$totalRows_rsHealthFacility = mysql_num_rows($rsHealthFacility);
?>
<?php include('Connections/cnKloAgogo.php'); ?>
<?php 
if(!isset($_SESSION))
...
<?php
mysql_free_result($rsHealthFacility);
?>
